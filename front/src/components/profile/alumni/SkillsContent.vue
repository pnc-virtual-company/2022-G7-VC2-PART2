<template>
    <section>
        <div class="" v-if="listSkill.length <= 0 && isEdit">
            <card-widget class="-mt-2">
                <template #body>
                    <div  @click="isEdited" class="flex justify-center items-center border-2 rounded-[2.5px] border-primary hover:bg-blue-100 cursor-pointer border-dashed">
                        <div class="text-center flex flex-col py-3">
                            <img src="../../../assets/default_img/skill.png" alt="" class="w-[60px] m-auto">
                            <h1 class="font-bold">Add skills here</h1>
                        </div>
                    </div>
                </template>
            </card-widget>
...
<script>
  ...
  components: { EditIcon },
        ...
        methods:{
            addNewSkill(){
                let skills = {name:this.skill ,type:this.radioType,alumni_id: this.user_id};
