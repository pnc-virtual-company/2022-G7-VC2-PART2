<template>
    <section class="w-10/12 m-auto" v-if="isFetch" v-cloak>
        ...
        <div class="w-7/12 mr-[8.4%] right-0 -mt-10">
            <general-info :alumniData="user" :major="major" :batch="batch"/>
            <education-background :alumniId="userId">
            </education-background>
            <work-experiences :alumniId="userId">
            </work-experiences>
        </div>
        </div>
    </section>
...
export default {
  ...
  methods: {
    async getAlumin() {
      await axios.get("alumni/"+this.userId).then((resp) => {
    ...
    async getSkillInfor() {
      await axios.get("/skills/alumni/" + this.userId).then((res) => {
