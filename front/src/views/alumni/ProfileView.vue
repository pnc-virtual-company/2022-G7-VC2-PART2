            ...
            <div class="mt-4 relative font-poppins">
                ...
                <div class="flex w-full relative">
                    <div class="w-[14%] relative ml-16 -mt-20">
                        ...
                        <input id="cover-img" hidden type="file" accept="image/*" @change="onchangeProfile" />
                    </div>
                    <div class="mt-[8%] block text-center absolute ml-[6%]">
                        <h1 class="text-2xl font-extrabold">{{data.firstName}} {{data.lastName}}</h1>
                                    ...
                                    <div class="absolute right-0 top-0 mr-2 mt-1 cursor-pointer">
                                        ...
                                        <div v-if="index==currentIndex" class="absolute bg-bgColorWhite space-y-1 p-1 rounded-md z-10"> 
                                            <div class="flex items-center hover:text-primary text-slate-400 text-sm">
                                                <edit-icon @click="showEdit"></edit-icon>
    ...
    export default {
        ...
        methods:{
            ...
            updateUserprofile(){
                ...
                axios.post('/users/updateimage/1',formdata).then((response)=>{
                    if (response.status==200){
                        this.updateImagealert('profile');
                        this.getAlumniimage();
                    }
                })
            },
            updateImagealert(message){
                let updatetitle = "";
                if (message == "profile"){
                    updatetitle = "User profile update successfull";
                }else {
                    updatetitle = "User cover update successfull";
                }
                if (message){
                    Swal.fire({
                        position: "center",
            ...
            async getAlumniimage(){
                await axios.get('alumni/2').then((response)=>{
                    this.profileurl = response.data.user.profile;
                    this.coverurl = response.data.user.cover;
                })
            },
            openOpenCard(){
