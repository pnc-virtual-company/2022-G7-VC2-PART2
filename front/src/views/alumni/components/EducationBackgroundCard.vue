<template>
  <div class="w-[100%] flex flex-wrap">
    ...
    <base-panel @action="isLimited=!isLimited">
    <template #header>
      ...
      <h1 class="text-extrabold text-sky-300 text-xl ml-2">
         Education Backgound
      </h1>
      </div>
      <add-icon class="text-primary" @click="openAddedu" />
    ...
    <template #limit>
          <school-card v-for="(school, index) in schoolFilter" :key="index" :school="school" @action="getSchoolId"/>
    </template>
  </base-panel>
        <edit-school-form :school="schoolEdit[0]" v-if="isShow"  @closeFormschoolBg="closeFormschoolBg">
        <template #hidden-form>
           <cancel-icon @click="isShow = !isShow"/>
        </template>
    </edit-school-form>
